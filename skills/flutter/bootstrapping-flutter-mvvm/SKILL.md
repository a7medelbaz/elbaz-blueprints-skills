...
Choose routing before step 1: named routes (simple, no deep links) or `go_router` (deep linking, web, nested nav)? Ask if unclear — see [reference/routing.md](reference/routing.md).

After applying, the project has `core/config`, `core/di`, `core/router`, `core/error`, `core/theme`, `core/utils`, `lib/app.dart`, `lib/main_dev.dart`, `lib/main_prod.dart`, Android flavors, CI workflow, Makefile, `.env`, and a wired `assets/` tree. Full tree: [reference/conventions.md](reference/conventions.md#target-project-structure).
...
- Theming — extending `AppColors`, `CustomColors`, `AppTextStyles` → [reference/theming.md](reference/theming.md)
...
The script creates `assets/images/`, `assets/svgs/`, `assets/fonts/`, `assets/translations/` and registers images/svgs/translations **folder-level** in `pubspec.yaml` — drop a file in and it is bundled, no pubspec edit needed. `assets/fonts/` gets a commented `# fonts:` template instead, since a font's family name and weights can't be inferred from an empty folder.

Localization is wired end-to-end: `en.json` is seeded, both entry points call `EasyLocalization.ensureInitialized()` and wrap `App()`, and `app.dart` passes the delegates/locale into `MaterialApp`. To add a locale, create `assets/translations/<locale>.json` and add it to `supportedLocales` in **both** `main_dev.dart` and `main_prod.dart`. Key namespacing: [reference/conventions.md](reference/conventions.md#localization).
